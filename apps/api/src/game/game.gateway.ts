  ...
  handleDisconnect(client: Socket): void {
    ...
    if (this.buildRoomStatePayload) {
      void this.buildRoomStatePayload(roomCode)
        ...
        .catch((err) => {
          this.logger.warn(
            `Failed to broadcast RoomStateUpdated after disconnect for ${roomCode}: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
    }
